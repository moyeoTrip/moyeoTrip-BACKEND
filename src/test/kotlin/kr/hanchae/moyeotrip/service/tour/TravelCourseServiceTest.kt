...
class TravelCourseServiceTest {
    ...
    private val userTripHistoryRepository = mock(UserTripHistoryRepository::class.java)
    private val service = TravelCourseService(tagRepository, roomRepository, courseRepository, userTripHistoryRepository)
    ...
    fun `완료한 커스텀 코스를 이름과 소개를 수정해 익명으로 공개한다`() {
        ...
        `when`(userTripHistoryRepository.existsByUserIdAndTravelCourseIdAndHostTrue(course.owner!!.id, course.id)).thenReturn(true)
